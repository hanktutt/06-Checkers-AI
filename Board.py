...
class Board:
	...
	def __init__(self, size, dimensions, colors, alternate, highlight):
		...
		for r in range(rows):
			...
			temp = []
			for c in range(cols):
				square = Square((c, r), self.colors[pos % len(self.colors)], self.alternate[pos % len(self.alternate)],
								self.highlight, (width // cols, height // rows))
				temp.append(square)
